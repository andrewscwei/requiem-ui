 ...
 *
 * @alias module:requiem-ui~ui.MasonryGrid
 */
class MasonryGrid extends ui.Element(HTMLElement) {
  /** @inheritdoc */
  ...
  set padding(value) {
    ...
    this.setDirty(enums.DirtyType.LAYOUT);
  }
  ...
  get maxWidth() {
    ...
    let ref = (parentNode ? utils.getRect(parentNode).width : 0) - refPadding;
  ...
  set maxWidth(value) {
    ...
    this.setDirty(enums.DirtyType.SIZE);
  }
  ...
  get maxHeight() {
    ...
    let ref = (parentNode ? utils.getRect(parentNode).height : 0) - refPadding;
  ...
  /**
   * enums.Orientation of the grid, either portrait or landscape.
   *
   * @type {enums.Orientation}
   * @see module:requiem~enums.enums.Orientation
   */
  get orientation() {
  ...
  /** @inheritdoc */
  update() {
    if (this.isDirty(enums.DirtyType.SIZE|enums.DirtyType.LAYOUT)) {
      this.reposition();
    }
  ...
  reposition() {
    ...
    for (let i = 0; i < l; i++) {
      ...
      if (!isNaN(this.itemWidth)) utils.transform(item, { width: this.itemWidth });
      if (!isNaN(this.itemHeight)) utils.transform(item, { height: this.itemHeight });
      ...
      utils.translate(item, { x: slot.x, y: slot.y });
    ...
    if (!this.autoResize) {
      if (!isNaN(mw) && this.orientation === enums.Orientation.PORTRAIT) {
    ...
    utils.transform(this, { width: isNaN(w) ? 0 : w, height: isNaN(h) ? 0 : h });
  }
  ...
  __computeItemPosition(item, vacancies) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (this.orientation === enums.Orientation.PORTRAIT) {
